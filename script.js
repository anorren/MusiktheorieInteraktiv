...
var FLAT_NAMES=['C','D♭','D','E♭','E','F','F♯','G','A♭','A','B','H'];
function s2nameScale(s){var m=((s%12)+12)%12;if(SHARPS.indexOf(m)>=0)return DE_NAMES[m];return DE_NAMES[m];}
...
(function(){
  ...
  var COLORS=[
    '#e53935','#fb8c00','#f9a825','#7cb342','#43a047','#00897b',
    '#0288d1','#1565c0','#283593','#4527a0','#6a1b9a','#ad1457'
  ];

  function drawQZ(){
    var svg=document.getElementById('qz-svg');
    if(!svg)return;
    svg.innerHTML='';
    ...
    // Draw segments
    for(var i=0;i<12;i++){
      var angle1=(i*30-105)*Math.PI/180;
      var angle2=((i+1)*30-105)*Math.PI/180;
      var midAngle=((i+0.5)*30-105)*Math.PI/180;

      // Outer segment (Dur)
      var path=document.createElementNS(SVG_NS,'path');
      var x1o=cx+rOuter*Math.cos(angle1),y1o=cy+rOuter*Math.sin(angle1);
      var x2o=cx+rOuter*Math.cos(angle2),y2o=cy+rOuter*Math.sin(angle2);
      var x1m=cx+rMid*Math.cos(angle1),y1m=cy+rMid*Math.sin(angle1);
      var x2m=cx+rMid*Math.cos(angle2),y2m=cy+rMid*Math.sin(angle2);
      path.setAttribute('d','M'+x1m+','+y1m+' L'+x1o+','+y1o+' A'+rOuter+','+rOuter+' 0 0,1 '+x2o+','+y2o+' L'+x2m+','+y2m+' A'+rMid+','+rMid+' 0 0,0 '+x1m+','+y1m);
      path.setAttribute('fill',COLORS[i]);path.setAttribute('opacity','0.15');
      ...
      (function(idx,p){
        p.addEventListener('mouseenter',function(){p.setAttribute('opacity','0.35');});
        p.addEventListener('mouseleave',function(){p.setAttribute('opacity','0.15');});
        p.addEventListener('click',function(){playArr(DUR[idx].semi,null);bubbleEl(svg.parentElement);});
      })(i,path);
      svg.appendChild(path);

      // Dur label
      var lx=cx+(rOuter+rMid)/2*Math.cos(midAngle);
      var ly=cy+(rOuter+rMid)/2*Math.sin(midAngle);
      var txt=document.createElementNS(SVG_NS,'text');
      txt.setAttribute('x',lx);txt.setAttribute('y',ly+1);
      txt.setAttribute('text-anchor','middle');txt.setAttribute('dominant-baseline','central');
      txt.setAttribute('font-size','18');txt.setAttribute('font-weight','900');
      txt.setAttribute('fill',COLORS[i]);txt.setAttribute('pointer-events','none');
      txt.textContent=DUR[i].name;
      svg.appendChild(txt);

      // Vorzeichen-Info
      var vzTxt=document.createElementNS(SVG_NS,'text');
      vzTxt.setAttribute('x',lx);vzTxt.setAttribute('y',ly+16);
      vzTxt.setAttribute('text-anchor','middle');vzTxt.setAttribute('font-size','9');
      vzTxt.setAttribute('font-weight','600');vzTxt.setAttribute('fill',COLORS[i]);
      vzTxt.setAttribute('opacity','0.7');vzTxt.setAttribute('pointer-events','none');
      if(DUR[i].sharps>0) vzTxt.textContent=DUR[i].sharps+'♯';
      else if(DUR[i].flats>0) vzTxt.textContent=DUR[i].flats+'♭';
      else vzTxt.textContent='–';
      svg.appendChild(vzTxt);

      // Inner segment (Moll)
      var pathM=document.createElementNS(SVG_NS,'path');
      var x1i=cx+rInner*Math.cos(angle1),y1i=cy+rInner*Math.sin(angle1);
      var x2i=cx+rInner*Math.cos(angle2),y2i=cy+rInner*Math.sin(angle2);
      var x1c=cx+rCenter*Math.cos(angle1),y1c=cy+rCenter*Math.sin(angle1);
      var x2c=cx+rCenter*Math.cos(angle2),y2c=cy+rCenter*Math.sin(angle2);
      pathM.setAttribute('d','M'+x1c+','+y1c+' L'+x1i+','+y1i+' A'+rInner+','+rInner+' 0 0,1 '+x2i+','+y2i+' L'+x2c+','+y2c+' A'+rCenter+','+rCenter+' 0 0,0 '+x1c+','+y1c);
      pathM.setAttribute('fill',COLORS[i]);pathM.setAttribute('opacity','0.08');
      pathM.setAttribute('stroke','white');pathM.setAttribute('stroke-width','2');
      pathM.style.cursor='pointer';pathM.style.transition='opacity .15s';
      (function(idx,p){
        p.addEventListener('mouseenter',function(){p.setAttribute('opacity','0.25');});
        p.addEventListener('mouseleave',function(){p.setAttribute('opacity','0.08');});
        p.addEventListener('click',function(){playArr(MOLL[idx].semi,null);bubbleEl(svg.parentElement);});
      })(i,pathM);
      svg.appendChild(pathM);

      // Moll label
      var mlx=cx+(rInner+rCenter)/2*Math.cos(midAngle);
      var mly=cy+(rInner+rCenter)/2*Math.sin(midAngle);
      var mtxt=document.createElementNS(SVG_NS,'text');
      mtxt.setAttribute('x',mlx);mtxt.setAttribute('y',mly+1);
      mtxt.setAttribute('text-anchor','middle');mtxt.setAttribute('dominant-baseline','central');
      mtxt.setAttribute('font-size','13');mtxt.setAttribute('font-weight','700');
      mtxt.setAttribute('fill',COLORS[i]);mtxt.setAttribute('opacity','0.65');
      mtxt.setAttribute('pointer-events','none');
      mtxt.textContent=MOLL[i].name;
      svg.appendChild(mtxt);

      // Separator line mid ring
      var sepLine=document.createElementNS(SVG_NS,'line');
      sepLine.setAttribute('x1',cx+rMid*Math.cos(angle1));sepLine.setAttribute('y1',cy+rMid*Math.sin(angle1));
      sepLine.setAttribute('x2',cx+rCenter*Math.cos(angle1));sepLine.setAttribute('y2',cy+rCenter*Math.sin(angle1));
      sepLine.setAttribute('stroke','white');sepLine.setAttribute('stroke-width','1.5');
      svg.appendChild(sepLine);
    }

    // Center circle
    var cc=document.createElementNS(SVG_NS,'circle');
    cc.setAttribute('cx',cx);cc.setAttribute('cy',cy);cc.setAttribute('r',rCenter);
    cc.setAttribute('fill','white');cc.setAttribute('stroke','#e0e0e0');cc.setAttribute('stroke-width','1');
    svg.appendChild(cc);
    var cLabel=document.createElementNS(SVG_NS,'text');
    cLabel.setAttribute('x',cx);cLabel.setAttribute('y',cy-8);
    cLabel.setAttribute('text-anchor','middle');cLabel.setAttribute('font-size','14');
    cLabel.setAttribute('font-weight','900');cLabel.setAttribute('fill','#333');
    cLabel.textContent='Quinten-';
    svg.appendChild(cLabel);
    var cLabel2=document.createElementNS(SVG_NS,'text');
    cLabel2.setAttribute('x',cx);cLabel2.setAttribute('y',cy+12);
    cLabel2.setAttribute('text-anchor','middle');cLabel2.setAttribute('font-size','14');
    cLabel2.setAttribute('font-weight','900');cLabel2.setAttribute('fill','#333');
    cLabel2.textContent='zirkel';
    svg.appendChild(cLabel2);

    // Ring labels
    var durLabel=document.createElementNS(SVG_NS,'text');
    durLabel.setAttribute('x',cx);durLabel.setAttribute('y',cy-rOuter-16);
    durLabel.setAttribute('text-anchor','middle');durLabel.setAttribute('font-size','11');
    durLabel.setAttribute('font-weight','700');durLabel.setAttribute('fill','#aaa');
    durLabel.setAttribute('letter-spacing','2');
    durLabel.textContent='DUR';
    svg.appendChild(durLabel);
    var mollLabel=document.createElementNS(SVG_NS,'text');
    mollLabel.setAttribute('x',cx+rInner+16);mollLabel.setAttribute('y',cy-2);
    mollLabel.setAttribute('text-anchor','start');mollLabel.setAttribute('font-size','9');
    mollLabel.setAttribute('font-weight','700');mollLabel.setAttribute('fill','#ccc');
    mollLabel.setAttribute('transform','rotate(90,'+cx+','+cy+')');
    mollLabel.textContent='MOLL';
    svg.appendChild(mollLabel);
  }
  drawQZ();
